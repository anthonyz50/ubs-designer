// ============================================================
// Component Renderers — live preview for each catalogue entry
// ============================================================
// Adapted from the component-showcase app's renderers.

import { useState, type ReactNode } from 'react';
import {
  FilterGroup, PageFooter, Header, MastheadNavigation, Links,
  ProcessNavigation, TreeNavigation, MenuListItem,
  Button, MenuButton, SplitButton, ButtonGroup, ButtonSelect,
  DropdownButton, ComboBox, ContextSelector, Dropdown, Slider,
  Switch, FileUpload, TimePicker, Checkbox, Radio, Toggle,
  Input, Select, Textarea, DatePicker,
  Accordion, AgendaSteps, Amount, Article, Avatar, BannerBox,
  Card, ContentBlock, CountryFlag, CreditCardDisplay, DataViz,
  Divider, List, Panel, SidebarNav, Stat, Table, Tabs, Tag,
  Chip, Tile, Typography,
  Alert, Badge, EmptyState, FormValidation, MessageBox, Modal,
  OverlayMessage, Popover, Progress, Skeleton, Snackbar, Tooltip, Timer,
  BarChart, DonutChartWidget, LineChart, PieChart,
  Grid, PageHeader, SectionWrapper, ActionBar, CTA, Logo, LogoTab,
  Icon, MovingFrame, Pattern,
  Breadcrumbs, Navbar, Pagination,
} from '@ubs/design-system';

// ─── Masthead Navigation Demo Components ──────────────────
...
export const componentRenderers: Record<string, RendererFn> = {
  ...
  PageFooter: (variant) => {
    const isDark = variant === 'dark' || variant === 'dark-minimal';
    const footerVariant = (variant === 'minimal' || variant === 'dark-minimal') ? 'minimal' : 'standard';
    const columns = [
      { title: 'Capabilities', links: [{ label: 'Wealth Management', href: '#' }, { label: 'Asset Management', href: '#' }, { label: 'Investment Bank', href: '#' }] },
      { title: 'About us', links: [{ label: 'Our firm', href: '#' }, { label: 'Investor Relations', href: '#' }, { label: 'Media', href: '#' }] },
      { title: 'Careers at UBS', links: [{ label: 'Job search', href: '#' }, { label: 'Working at UBS', href: '#' }] },
      { title: 'Contact', links: [{ label: 'Locations', href: '#' }, { label: 'Client login', href: '#' }] },
    ];
    return (
      <div style={{ width: '100%' }} data-ubs-theme={isDark ? 'dark' : undefined}>
        <PageFooter
          variant={footerVariant}
          columns={columns}
          legal="UBS Group AG is incorporated and domiciled in Switzerland and serves as the holding company."
          copyright="&copy; UBS 2024. All rights reserved."
        />
      </div>
    );
  },
  ...
  Header: (variant) => (
    <div style={{ width: '100%' }}>
      <Header
        ...
        logo={<img src="/ubs-logo.svg" alt="UBS" style={{ height: '22px', width: 'auto' }} />}
        navItems={[
  ...
  TreeNavigation: () => (
    <TreeNavigation
      items={[
        { id: '1', label: 'Portfolios', children: [{ id: '1-1', label: 'Conservative' }, { id: '1-2', label: 'Growth' }] },
        { id: '2', label: 'Reports', children: [{ id: '2-1', label: 'Monthly' }, { id: '2-2', label: 'Annual' }] },
      ]}
    />
  ),
  ...
  FileUpload: (variant) => (
    <div style={{ width: '100%', maxWidth: 400 }}>
      <FileUpload variant={variant === 'compact' ? 'compact' : 'dropzone'} />
    </div>
  ),
  ...
  DatePicker: () => <DatePicker label="Start date" />,
  ...
  Input: (variant) => (
    <div style={{ width: '100%', maxWidth: 300, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Input label="Email address" placeholder="Enter your email" size={variant === 'sm' ? 'sm' : variant === 'lg' ? 'lg' : 'md'} type="email" />
      <Input label="Amount" placeholder="0.00" size={variant === 'sm' ? 'sm' : variant === 'lg' ? 'lg' : 'md'} error="Please enter a valid amount" />
    </div>
  ),

  Select: () => (
    <div style={{ width: '100%', maxWidth: 300 }}>
      <Select label="Currency" options={[{ value: 'chf', label: 'CHF' }, { value: 'usd', label: 'USD' }, { value: 'eur', label: 'EUR' }]} placeholder="Select currency" />
    </div>
  ),

  Textarea: () => (
    <div style={{ width: '100%', maxWidth: 400 }}>
      <Textarea label="Notes" placeholder="Add your notes here..." rows={3} />
    </div>
  ),
  ...
  Card: (variant) => (
    <Card variant={variant === 'pastel1' ? 'pastel1' : variant === 'pastel2' ? 'pastel2' : variant === 'gray' ? 'gray' : 'default'} padding="medium" hoverable>
  ...
  ContentBlock: (variant) => (
    <ContentBlock title="Wealth Management" variant={variant === 'highlight' ? 'highlight' : variant === 'bordered' ? 'bordered' : 'default'}>
  ...
  CreditCardDisplay: () => (
    <CreditCardDisplay number="4242424242424242" name="John Smith" expiry="12/28" type="visa" variant="premium" />
  ),

  DataViz: () => (
    <DataViz type="bar" data={[{ label: 'Q1', value: 45 }, { label: 'Q2', value: 62 }, { label: 'Q3', value: 38 }, { label: 'Q4', value: 71 }]} />
  ),
  ...
  Stat: (variant) => (
    <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
      <Stat label="Portfolio Value" value="1,234,567" prefix="CHF " change={{ value: 2.34, direction: 'up' }} size={variant === 'sm' ? 'sm' : variant === 'lg' ? 'lg' : 'md'} />
      <Stat label="Daily P&L" value="-12,450" prefix="CHF " change={{ value: -0.8, direction: 'down' }} size={variant === 'sm' ? 'sm' : variant === 'lg' ? 'lg' : 'md'} />
    </div>
  ),
  ...
  Typography: (variant) => {
    const map: Record<string, string> = {
      h1: 'keyline', h2: 'subheadline1', h3: 'subheadline2', h4: 'subheadline3',
      body: 'copyText', caption: 'captions', overline: 'environmentalInfo',
    };
    const labels: Record<string, string> = {
      h1: 'Keyline Heading', h2: 'Subheadline 1', h3: 'Subheadline 2', h4: 'Subheadline 3',
      body: 'Body copy text implementing the UBS type scale.', caption: 'Caption text style', overline: 'OVERLINE TEXT',
    };
    const v = map[variant] ?? 'copyText';
    const t = labels[variant] ?? 'Body copy text implementing the UBS type scale.';
    return <Typography variant={v as any}>{t}</Typography>;
  },
  ...
  EmptyState: () => (
    <EmptyState title="No transactions found" description="There are no transactions matching your current filters." action={{ label: 'Clear filters', onClick: () => {} }} />
  ),
  ...
  MessageBox: (variant) => (
    <div style={{ width: '100%' }}>
      <MessageBox variant={(variant as any) || 'info'} title="Important Notice">
  ...
  Modal: () => (
    <div style={{ position: 'relative', width: '100%', height: 80, background: 'var(--ubs-bg-tertiary)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--ubs-text-secondary)' }}>
  ...
  Skeleton: (variant) => (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 12, alignItems: 'center' }}>
      {variant === 'circle' ? <Skeleton variant="circle" width={48} height={48} /> :
       variant === 'text' ? <Skeleton variant="text" lines={3} width="100%" /> :
  ...
  Snackbar: (variant) => (
    <div style={{ position: 'relative', width: '100%', minHeight: 60 }}>
      <Snackbar message="Changes saved successfully" variant={variant === 'success' ? 'success' : variant === 'error' ? 'error' : 'info'} open={true} />
    </div>
  ),
  ...
  Timer: (variant) => (
    <Timer direction="down" initialSeconds={3661} variant={variant === 'compact' ? 'compact' : 'default'} autoStart={false} />
  ),
  ...
  BarChart: (variant) => (
    <div style={{ width: '100%', minHeight: 200 }}>
      <BarChart
        ...
        data={[{ label: 'Q1', values: [42] }, { label: 'Q2', values: [58] }, { label: 'Q3', values: [35] }, { label: 'Q4', values: [71] }]}
        showValues xLabel="Quarter" yLabel="Revenue (M)"
      />
    </div>
  ),
  ...
  DonutChartWidget: () => (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <DonutChartWidget
        data={[{ label: 'Equities', value: 45 }, { label: 'Bonds', value: 30 }, { label: 'Alternatives', value: 15 }, { label: 'Cash', value: 10 }]}
        size={180} centerLabel="Portfolio" showPercentages
      />
    </div>
  ),
  ...
  LineChart: () => (
    <div style={{ width: '100%', minHeight: 200 }}>
      <LineChart
        ...
        showPoints showGrid xLabel="Month" yLabel="Value"
      />
    </div>
  ),
  ...
  PieChart: () => (
    <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
      <PieChart
        data={[{ label: 'EMEA', value: 40 }, { label: 'Americas', value: 35 }, { label: 'APAC', value: 20 }, { label: 'Other', value: 5 }]}
        size={180} showLabels showLegend
      />
    </div>
  ),
  ...
  Grid: () => (
    <Grid columns={3} gap="medium">
      {[1, 2, 3, 4, 5, 6].map((n) => (
        <div key={n} style={{ background: 'var(--ubs-bg-tertiary)', padding: 16, borderRadius: 4, textAlign: 'center', fontSize: 13 }}>Cell {n}</div>
      ))}
    </Grid>
  ),

  Layout: () => (
    <div style={{ width: '100%', height: 100, border: '1px dashed var(--ubs-border)', borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: 'var(--ubs-text-secondary)' }}>
  ...
  PageHeader: (variant) => (
    <div style={{ width: '100%' }}>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Home', href: '#' }, { label: 'Investments', href: '#' }, { label: 'Portfolio', href: '#' }]}
      />
    </div>
  ),
  ...
  ActionBar: () => (
    <div style={{ width: '100%' }}>
      <ActionBar primaryAction={{ label: 'Submit', onClick: () => {} }} secondaryAction={{ label: 'Cancel', onClick: () => {} }} />
    </div>
  ),
