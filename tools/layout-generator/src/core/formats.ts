...
function scaleImpulse(widthMm: number): { impulseWidth: number; impulseSpacing: number } {
...
export const FORMAT_SPECS: Record<ISOFormat, FormatSpec> = {
  A0: {
    name: 'A0',
    width: 841,
    height: 1189,
 ...
 */
export const COLOUR_PALETTES: Record<ColourDirection, ColourPalette> = {
  gray: {
    name: 'gray',
    colours: {
      'white': '#FFFFFF',
...
export function getFormatSpec(format: ISOFormat): FormatSpec {
  return FORMAT_SPECS[format];
}

export function getColourPalette(direction: ColourDirection): ColourPalette {
  return COLOUR_PALETTES[direction];
}
