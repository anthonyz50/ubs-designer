 ...
 */
function findNearestPaletteColour(hex) {
 ...
 */
function validateCSSContent(content, file, fix) {
    const issues = [];
    let ast;
    try {
                            ...
                            csstree.walk(valueNode, {
                                enter(child) {
                                    if (child.type === 'Number') {
                                        args.push(parseFloat(child.value));
                                    }
                                    if (child.type === 'Percentage') {
                                        // Convert percentage to 0-255 for RGB
                                        args.push(Math.round((parseFloat(child.value) / 100) * 255));
                                    }
                                },
                            });
                            if (args.length >= 3) {
...
//# sourceMappingURL=colour-check.js.map