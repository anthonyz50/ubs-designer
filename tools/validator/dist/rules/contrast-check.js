 ...
 */
function extractColourFromValue(valueNode) {
    let result = null;
                    ...
                    csstree.walk(node, {
                        enter(child) {
                            if (child.type === 'Number') {
                                nums.push(parseFloat(child.value));
                            }
                        },
                    });
                    if (nums.length >= 3) {
...
function parseFontSize(value) {
    ...
    if (ptMatch)
        return parseFloat(ptMatch[1]) * (4 / 3); // pt to px conversion
    const remMatch = value.match(/([\d.]+)\s*rem/i);
    if (remMatch)
        return parseFloat(remMatch[1]) * 16; // assume 16px base
    const emMatch = value.match(/([\d.]+)\s*em/i);
    if (emMatch)
        return parseFloat(emMatch[1]) * 16; // approximate
    return null;
}
/**
...
function isLargeText(fontSize, fontWeight) {
    ...
    if (fontSize >= 24)
        return true;
    if (fontSize >= 18.66 && fontWeight !== null && fontWeight >= 700)
        return true;
    return false;
}
exports.contrastCheck = {
    name: 'contrast-check',
